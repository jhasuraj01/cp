...
double cost(vec(double) &X, vec(double) &T, double x0) {
    double ans = 0;
    int n = X.size();
...
void solution() {
    ...
    double left = *min_element(all(X));
    double right = *max_element(all(X));
    double ans = INT_MAX;

    while(abs(right - left) > 1e-7) {
        double mid = (left + right)/2;
        double midl = (2*left + right)/3;
        double midr = (left + 2*right)/3;

        double costl = cost(X, T, midl);
        double costr = cost(X, T, midr);
