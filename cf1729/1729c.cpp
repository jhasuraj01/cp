...
void solution() {
    ...
    pair<char, int> first = A[0];
    pair<char, int> last = A.back();
    int cost = A.back().first - A[0].first;

    sort(all(A));

    int start = lower_bound(all(A), first) - A.begin();
    int end = upper_bound(all(A), last) - A.begin();
    int steps = end - start;
    ...
    for (int i = start; i < end; i++) {
        ans.push_back((reversed ? n + 1 - A[i].second : A[i].second));
    }

    if(reversed) reverse(all(ans));
